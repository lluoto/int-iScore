...
class_con = freesasa.Classifier("oons_md.config")
...
def convert_and_soap(model,reference):
    ...
    for chain_id in chains:
        sel = Selection(chain_id)  # 创建当前链的选择对象
        dope = sel.assess_dope(normalize=True)  # 评估归一化 DOPE
    ...
    return score,model_chains,reference_chains,new_path,sum(dope_scores)
@time_comsuming_printer
