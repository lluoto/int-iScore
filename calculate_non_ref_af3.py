...
def count_clashes(structure,chain_info, clash_cutoff=0.4):
    ...
    query_chains = chain_info[0] if isinstance(chain_info[0], list) else [chain_info[0]]
    partner_chains = chain_info[1] if isinstance(chain_info[1], list) else [chain_info[1]]
    ...
    sasa_chain_A = calculate_sasa(q_atoms) if q_atoms else 0
    sasa_chain_B = calculate_sasa(p_atoms) if p_atoms else 0
    complex_atoms = q_atoms + p_atoms
    sasa_complex = calculate_sasa(complex_atoms) if complex_atoms else 0
    bsa=(sasa_chain_A+sasa_chain_B-sasa_complex)/2
...
input_dir='/media/cuixi/data0/lluoto/5-HT2C-2CD28'
# Example usage
pdb_files = [os.path.join(input_dir,file) for file in os.listdir(input_dir) if '.cif' in file]  # List of 1000 PDB files
...
# Write the structure to a PDB file
    ...
    new_path=model.replace('.cif','.pdb')
    ...
    return score,model_chains,dope,new_path
...
def process_json_files(output_file):
    ...
    # Each process processes its assigned files
        for file_path in pdb_files:
            ...
            soap,model_chain,dope,new_name=convert_and_soap(file_path)

            cp_score,interface_residues,pairs=dp2_and_cpscore(new_name,model_chain)
            structure = pdbfile_parser.get_structure('0',new_name)
            ...
            with open(output_file, 'a',newline='') as out_file:
                ...
                output_line=[split_file[-1],dope,soap,frustration_score,cp_score,bsa,interface_residues,pairs,rama,f'rank {0} do this']
                print(output_line)
                w.writerow(output_line)
            os.system(f'cp {new_name} {temp_dir}')
    except TypeError:
        pass

# input_file_path='rpm/6OFJ/'
# input_file_path='/home/lluoto/chan_g/benchmark/'
output_file = '5_HT2C_1_3.csv'
final_output_file = '5_HT2C_3_3.csv'
with open(final_output_file, 'w',newline='') as out_file:
    w=csv.writer(out_file)
    output_line=['filename','dope','soap','dockq','frustration_score','cpscore','bsa','sc','ec']
    w.writerow(output_line)
